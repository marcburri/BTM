...
#' @param a numeric, indicating the symmetric dirichlet prior probability of a topic P(z). Defaults to 50/k.
#' @param b numeric, indicating the symmetric dirichlet prior probability of a word given the topic P(w|z). Defaults to 0.01.
...
#' x <- x[, c("doc_id", "lemma")]
#' model  <- BTM(x, k = 5, a = 1, b = 0.01, iter = 10, trace = TRUE)
#' model
#' terms(model)
...
#' set.seed(123456)
#' model <- BTM(x, k = 5, b = 0.01, iter = 10, background = TRUE)
#' model
#' terms(model)
#'
#' ##
...
print.BTM <- function(x, ...){
  ...
  cat(sprintf("  trained with %s Gibbs iterations, a: %s, b: %s", x$iter, x$a, x$b), sep = "\n")
